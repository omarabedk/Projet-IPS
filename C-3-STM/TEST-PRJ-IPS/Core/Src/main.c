...
double Consigne = 100; //En cm par rapport au sol !!
double Mesure_sortie; //Mesure du capteur conversion en cm !!!
double error; //Ecart : error = Consigne - Mesure_sortie
double Kp=1.14;       //1.136; //Gain correcteur proportionel
double Ki=0.01; //Gain correcteur intégrale
...
float Recevoir_tension_SHUNT(ADC_HandleTypeDef* hadc, UART_HandleTypeDef* huart) {
	 ...
	 power = ((pow((((raw_value / 4095.0)*3.3)-0.8),2)/0.05)-0.27)*10;
...
void Correcteur(UART_HandleTypeDef* huart){
	...
	__HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_1, up+ui);
...
void send_data(UART_HandleTypeDef *huart, float power, int position) {
    char buffer[50];
    sprintf(buffer, "P%.2f;H%d;", power, position);
...
int main(void)
{
  ...
  while (1) {
	  	  	power = Recevoir_tension_SHUNT(&hadc1,&huart2); // Implement this function
            position = get_position(&huart2); // Implement this function
            send_data(&huart2, power, position);
            Correcteur(&huart2);
             // Add delay to control data transmission rate
        }
  /* USER CODE END 3 */
}
...
{
    int receivedValue = 0;
    int i = 1;  // Start from index 1, assuming index 0 is the command character

    // Loop through all the digits in the Rx_data array until we hit a non-digit character
    while (Rx_data[i] >= '0' && Rx_data[i] <= '9')
    {
        receivedValue = receivedValue * 10 + (Rx_data[i] - '0'); // Convert ASCII to integer and add to value
        i++;
    }
...
double get_value_from_digits_double()
{
    ...
    // Loop through all the digits and the decimal point in the Rx_data array
    while ((Rx_data[i] >= '0' && Rx_data[i] <= '9') || Rx_data[i] == '.')
    {
        if (Rx_data[i] == '.')
        {
            if (decimal_found) {
                // If we encounter a second decimal point, break the loop
                break;
            }
            decimal_found = 1;
        }
        else
        {
            if (decimal_found)
            {
                ...
                receivedValue += (Rx_data[i] - '0') / decimal_divisor;
            }
            else
            {
                // Processing the integer part
                receivedValue = receivedValue * 10.0 + (Rx_data[i] - '0');
            }
        }
        i++;
    }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if(huart == &huart2){
		 ...
		 if(Rx_data[0]=='H'){ //Recevoir Kp sur 4 chiffres entiers ( jusqu'à 9999 )
			 Consigne= get_value_from_digits();
		 } else if(Rx_data[0]=='G'){ //Recevoir Ki sur 4 chiffres entiers ( jusqu'à 9999 )
			 Ki=get_value_from_digits_double();
		 }else{
			 return;
		 }
